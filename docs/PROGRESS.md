...
* The app now shows the icon's mark (`Logo`) instead of the letter monogram, on Today and in
  Settings. Onboarding opens with the wordmark (ADR-0027).
* SVG logo and wordmarks (English sans, English serif, Persian) in `packaging/icon/`, generated by
  `make_logo.py`.
* Settings › Appearance has a font picker for each language. English: Sans, Serif, Book, Legible.
  Persian: Vazirmatn, Plex, Naskh, Markazi. The choice is device-local and applies to the whole
  app, pages included.
* `tools/fetch_fonts.py` fetches and cuts the fonts.
...
* Flutter: 229 tests, 72 goldens rendered on Linux (`tools/update_goldens.sh` runs them in Docker
  from any host).
