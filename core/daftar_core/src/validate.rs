...
pub fn is_claim_zone(path: &str) -> bool {
    path.starts_with("vaults/health/")
        || path.starts_with("vaults/mind/")
        || path == "vaults/life/profile.md"
        || path.starts_with("vaults/life/concerns/")
}
...
pub fn validate(
    ...
    cs: &Changeset,
    scope: &Scope,
    human_lines: &dyn Fn(&str) -> BTreeSet<usize>,
) -> Report {
    ...
    for (path, content) in &cs.files {
        if layout::is_protected_from_ai(path) {
            r.errors.push(format!(
                "{path}: this path may not be written by the assistant."
            ));
            continue;
        }
        let vault = pages::vault_of(path).unwrap_or_default();
        match scope {
            Scope::Personal if vault == "stories" => r.errors.push(format!(
                "{path}: non-fiction content may not be written into stories/."
            )),
            Scope::Story(s) if !path.starts_with(&format!("vaults/stories/{s}/")) => r.errors.push(
                format!("{path}: fiction may only be written inside vaults/stories/{s}/."),
            ),
        ...
        if !wiki::PAGE_TYPES.contains(&page.meta.kind.as_str()) {
            r.errors.push(format!(
                "{path}: frontmatter type '{}' is not a known page type.",
                page.meta.kind
            ));
        }
        if page.meta.title.en.trim().is_empty() || page.meta.title.fa.trim().is_empty() {
            r.errors
                .push(format!("{path}: title needs both en and fa."));
        }
        if page.meta.summary.trim().is_empty() && !stub {
            r.errors.push(format!(
                "{path}: summary is empty; it feeds the vault index."
            ));
        }
        if page.meta.vault != vault {
            r.errors.push(format!(
                "{path}: frontmatter vault '{}' does not match the folder.",
                page.meta.vault
            ));
        }
        ...
        if n > MAX_PAGE_LINES {
            r.warnings
                .push(format!("{path} has {n} lines; consider splitting it."));
        }
        ...
        let old_targets: BTreeSet<String> = wiki::links(&old_body)
            .into_iter()
            .map(|l| l.target)
            .collect();
        ...
        if !stub {
            ...
            for (ln, line) in &added {
                ...
                let cites = text
                    .iter()
                    .any(|l| wiki::links(l).iter().any(|k| k.target.starts_with("raw/")));
        ...
        if is_claim_zone(path) {
            ...
            for (ln, line) in &added {
                ...
                let sec = secs
                    ...
                    .filter(|s| s.start <= *ln && *ln <= s.end)
                    .max_by_key(|s| s.level)
                    .map(|s| normalize(&s.heading));
                if sec
                    .as_deref()
                    .is_some_and(|h| NON_CLAIM_SECTIONS.iter().any(|n| normalize(n) == h))
                {
                    continue;
                }
        ...
        if !old.is_empty() {
            ...
            if !human.is_empty() {
                ...
                for ln in removed_lines(&old, new) {
                    if ln > fm_end
                        && human.contains(&ln)
                        && old_lines.get(ln - 1).is_some_and(|l| !l.trim().is_empty())
                    {
