...
use crate::providers::{
    self, AiConfig, DynProvider, ProviderError, ProviderErrorKind, Role, RoleConfig,
};
...
impl AiRuntime {
    ...
    pub fn has(&self, role: Role) -> bool {
        self.config
            .role(role)
            .is_some_and(|r| self.config.provider(&r.provider).is_some())
    }
    ...
    pub fn for_role(&self, role: Role) -> Result<(DynProvider, RoleConfig), ProviderError> {
        let rc = self.config.role(role).cloned().ok_or_else(|| {
            ProviderError::new(
                ...
                format!(
                    "No model is set up for {}. Choose one in Settings › Models.",
                    role.as_str()
                ),
            )
        })?;
        ...
        let pc = self.config.provider(&rc.provider).ok_or_else(|| {
            ProviderError::new(
                ProviderErrorKind::NotConfigured,
                format!(
                    "The provider for {} was removed. Choose another in Settings › Models.",
                    role.as_str()
                ),
            )
        })?;
