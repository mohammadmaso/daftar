...
use super::sse;
use super::{
    ChatRequest, ChatResponse, LlmProvider, MsgRole, OnDelta, Part, ProviderConfig, ProviderError, ProviderResult,
    StopReason, ToolCall,
};
...
impl Gemini {
    pub fn new(c: &ProviderConfig, key: String, timeout: Duration) -> Self {
        let base = if c.base_url.is_empty() { "https://generativelanguage.googleapis.com/v1beta".to_owned() } else { c.base_url.trim_end_matches('/').to_owned() };
        Self { name: c.name.clone(), base, key, headers: c.extra_headers.clone(), http: crate::tls::http_client(timeout) }
    }

    fn req(&self, method: reqwest::Method, path: &str) -> reqwest::RequestBuilder {
        let mut r = self.http.request(method, format!("{}{path}", self.base)).header("x-goog-api-key", &self.key);
    ...
    pub(crate) fn body(req: &ChatRequest) -> Value {
        ...
        for m in &req.messages {
            let (role, parts): (&str, Vec<Value>) = match m.role {
                MsgRole::User => (
                    ...
                    m.parts
                        ...
                        .map(|p| match p {
                            ...
                            Part::Image { media_type, data } => json!({"inline_data": {"mime_type": media_type, "data": data}}),
                        })
                        .collect(),
                ),
                MsgRole::Assistant => {
                ...
                MsgRole::Tool => ("user", vec![json!({"functionResponse": {"name": m.tool_name, "response": {"content": m.text()}}})]),
            };
...
fn strip_unsupported(v: &Value) -> Value {
    match v {
        Value::Object(o) => Value::Object(
            o.iter()
                .filter(|(k, _)| !matches!(k.as_str(), "additionalProperties" | "$schema" | "default"))
...
impl LlmProvider for Gemini {
    ...
    async fn chat(&self, req: &ChatRequest, on_delta: OnDelta<'_>) -> ProviderResult<ChatResponse> {
        ...
        let resp = self.req(reqwest::Method::POST, &path).json(&Self::body(req)).send().await.map_err(|e| ProviderError::from_reqwest(&self.name, e))?;
        ...
        sse::read(&self.name, resp, |ev| {
            let Ok(v) = serde_json::from_str::<Value>(&ev.data) else { return Ok(()) };
            if let Some(u) = v.get("usageMetadata") {
                usage.input_tokens = u["promptTokenCount"].as_u64().unwrap_or(usage.input_tokens);
                usage.output_tokens = u["candidatesTokenCount"].as_u64().unwrap_or(usage.output_tokens);
            ...
            if let Some(parts) = v.pointer("/candidates/0/content/parts").and_then(Value::as_array) {
                for p in parts {
                    ...
                    if let Some(fc) = p.get("functionCall") {
                        calls.push(ToolCall { id: format!("call_{}", calls.len()), name: fc["name"].as_str().unwrap_or_default().to_owned(), arguments: fc["args"].clone() });
                    }
                }
            }
        ...
        Ok(ChatResponse { text, tool_calls: calls, usage, stop })
    }
    ...
    async fn embed(&self, model: &str, inputs: &[String]) -> ProviderResult<Vec<Vec<f32>>> {
        ...
        let resp = self.req(reqwest::Method::POST, &format!("/models/{model}:batchEmbedContents")).json(&json!({"requests": reqs})).send().await.map_err(|e| ProviderError::from_reqwest(&self.name, e))?;
        let v: Value = check(&self.name, resp).await?.json().await.map_err(|e| ProviderError::from_reqwest(&self.name, e))?;
        Ok(v["embeddings"].as_array().map(|a| a.iter().map(|e| e["values"].as_array().map(|x| x.iter().filter_map(Value::as_f64).map(|f| f as f32).collect()).unwrap_or_default()).collect()).unwrap_or_default())
    }

    async fn list_models(&self) -> ProviderResult<Vec<String>> {
        let resp = self.req(reqwest::Method::GET, "/models?pageSize=200").send().await.map_err(|e| ProviderError::from_reqwest(&self.name, e))?;
        let v: Value = check(&self.name, resp).await?.json().await.map_err(|e| ProviderError::from_reqwest(&self.name, e))?;
        Ok(v["models"].as_array().map(|a| a.iter().filter_map(|m| m["name"].as_str().map(|n| n.trim_start_matches("models/").to_owned())).collect()).unwrap_or_default())
    }
}
