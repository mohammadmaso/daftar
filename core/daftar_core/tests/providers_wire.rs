...
use daftar_core::providers::{self, ChatRequest, LlmProvider, Message, ProviderConfig, ProviderErrorKind, ProviderKind, StopReason, ToolSpec};
use serde_json::{Value, json};
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::TcpListener;

/// Serves one response and returns (base_url, request-body receiver).
async fn serve_once(status: u16, content_type: &str, body: String) -> (String, tokio::sync::oneshot::Receiver<(String, String)>) {
...
fn cfg(kind: ProviderKind, base: &str) -> ProviderConfig {
    ProviderConfig { id: "p".into(), name: "Test".into(), kind, base_url: base.into(), extra_headers: [("X-Title".to_string(), "Daftar".to_string())].into(), timeout_s: 10 }
}
...
fn request() -> ChatRequest {
    ChatRequest {
        ...
        tools: vec![ToolSpec { name: "search".into(), description: "find".into(), parameters: json!({"type": "object", "properties": {"q": {"type": "string"}}, "additionalProperties": false}) }],
        max_tokens: 256,
...
async fn openai_streams_text_tools_and_usage() {
    ...
    let p = providers::build(&cfg(ProviderKind::OpenaiCompatible, &url), Some("sk-test".into())).unwrap();
    ...
    assert_eq!((r.usage.input_tokens, r.usage.output_tokens, r.usage.cached_input_tokens), (12, 7, 4));

    let (head, body) = rx.await.unwrap();
    assert!(head.starts_with("POST /chat/completions"));
    assert!(head.to_lowercase().contains("authorization: bearer sk-test"));
...
async fn anthropic_streams_and_caches_system() {
    let events = [
        ("message_start", json!({"type": "message_start", "message": {"usage": {"input_tokens": 5, "cache_read_input_tokens": 100}}})),
        ...
        ("content_block_start", json!({"type": "content_block_start", "index": 1, "content_block": {"type": "tool_use", "id": "tu1", "name": "search"}})),
        ("content_block_delta", json!({"type": "content_block_delta", "index": 1, "delta": {"type": "input_json_delta", "partial_json": "{\"q\": \"x\"}"}})),
        ("message_delta", json!({"type": "message_delta", "delta": {"stop_reason": "tool_use"}, "usage": {"output_tokens": 9}})),
    ];
    let sse: String = events.iter().map(|(e, d)| format!("event: {e}\ndata: {d}\n\n")).collect();
    let (url, rx) = serve_once(200, "text/event-stream", sse).await;
    ...
    assert_eq!(r.stop, StopReason::ToolUse);
    assert_eq!((r.usage.input_tokens, r.usage.cached_input_tokens, r.usage.output_tokens), (105, 100, 9));
    let (head, body) = rx.await.unwrap();
...
async fn http_errors_become_human_messages() {
    let (url, _rx) = serve_once(401, "application/json", r#"{"error":{"message":"invalid x-api-key"}}"#.into()).await;
...
async fn openai_transcription_and_models() {
    let (url, rx) = serve_once(200, "application/json", r#"{"text": " امروز خسته بودم "}"#.into()).await;
    let p = providers::build(&cfg(ProviderKind::OpenaiCompatible, &url), Some("k".into())).unwrap();
    let t = p.transcribe("whisper-large-v3", b"abc".to_vec(), "a.m4a", Some("fa")).await.unwrap();
    assert_eq!(t, "امروز خسته بودم");
    let (head, body) = rx.await.unwrap();
    assert!(head.starts_with("POST /audio/transcriptions"));
    assert!(body.contains("name=\"model\"") && body.contains("whisper-large-v3") && body.contains("name=\"language\""));

    let (url, _rx) = serve_once(200, "application/json", r#"{"data":[{"id":"b"},{"id":"a"}]}"#.into()).await;
