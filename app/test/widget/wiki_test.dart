...
void main() {
  testWidgets('the Wiki tab searches and opens pages', (tester) async {
    ...
    expect(find.text('journal-day'), findsOneWidget, reason: 'property header, not raw YAML');
    expect(find.text('Edited on two devices'), findsNothing, reason: 'a titled callout shows its own title');
    expect(find.text('From laptop'), findsOneWidget);
  });

  testWidgets('wikilinks open pages; missing pages say so; backlinks list linkers', (tester) async {
    final lib = wikiLibrary();
    await pumpApp(tester, setup: FakeSetup(library: lib), location: '/wiki/page?path=${Uri.encodeQueryComponent(journal)}');
    await scrollTo(tester, find.text('Linked from'));
    expect(find.text('Sara'), findsOneWidget, reason: 'backlink row');

    final rich = find.byWidgetPredicate((w) => w is RichText && w.text.toPlainText().contains('nowhere'));
    ...
    expect(find.textContaining('(status::'), findsNothing, reason: 'inline fields are chips, not raw text');
  });

  testWidgets('editing saves one human edit and reports a stale page', (tester) async {
    final lib = wikiLibrary();
    await pumpApp(tester, setup: FakeSetup(library: lib), location: '/wiki/page?path=${Uri.encodeQueryComponent('vaults/life/people/sara.md')}');
    await tester.tap(find.bySemanticsLabel('Edit'));
    await tester.pumpAndSettle();
    final field = find.byType(EditableText);
    final text = tester.widget<EditableText>(field).controller.text;
    await tester.enterText(field, text.replaceFirst('Lives in Tehran.', 'Lives in Shiraz.'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Save'));
    await tester.pumpAndSettle();
    expect(lib.saved.single.$2, contains('Lives in Shiraz.'));
    expect(find.textContaining('Lives in Shiraz.'), findsOneWidget, reason: 'back on the reader, updated');

    await tester.tap(find.bySemanticsLabel('Edit'));
    await tester.pumpAndSettle();
    lib.addPage('vaults/life/people/sara.md', 'Sara', 'سارا', 'Changed on another device.', kind: 'person');
